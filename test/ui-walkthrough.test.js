...
test('the seeded walkthrough covers every stable route and popup trigger', () => {
  ...
  assert.ok(popupIds.length >= 40, `expected a full seeded popup map, found ${popupIds.length}`);
  ...
  assert.doesNotMatch(indexSource, /style\s*=/i, 'seeded mock must not use inline style attributes');
});
...
test('the popup contract is centered, modal, keyboard-closeable, and sample-labeled', () => {
  ...
  assert.match(indexSource, /Seeded UI preview/);
  assert.match(mainSource, /Sample data|sample data/);
});
