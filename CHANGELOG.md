...
- **F3 (blocking).** `verifyProvenancePanel` in `scripts/run-browser-harness.js` gained assertions for F1's visible value and F2's hash-field-tamper case. Chromium/Firefox execution was blocked in every sandboxed authoring session — `cdn.playwright.dev` is outside the reachable network allowlist there. Run for real on a machine with working network access: `npm run test:browser` initially failed a pre-existing fixture, `verifyDevOnlyDependency` — it copies `scripts`/`src`/`vendor` into a `.git`-free temp directory to prove the build needs no `devDependencies` (in particular, no Playwright at runtime), then asserts the result is byte-identical to the real build. That assertion predates P0.16 and implicitly assumed the build needs nothing outside `node_modules`; once the build date started reading `git log`, a `.git`-free copy legitimately produces a different (fallback "unknown") date than a real checkout, which still has its `.git`. Not a build regression — the fixture no longer modeled a real "node_modules absent" checkout, since a real one still has `.git`. Fixed by copying `.git` into the fixture too, restoring byte-for-byte equality without weakening what's being proven. Added an equivalent Playwright-free regression test, `test/build.test.js`'s "a build with node_modules absent but .git present matches the real build byte-for-byte", so this specific property is now covered by plain `npm test` and doesn't depend on a browser-capable environment to catch a regression. Confirmed this fix does not change `coldbox.html`'s hash (still `29cdda18eb583ea8d2226400f59ce01a772b883e84533c3706cc273e30d44deb`) — it only corrects the test fixture.
