...
### Added — P0.17 help framework (in progress, 2026-08-07)

**Not complete — recorded here per doc-hygiene.md rule 3 while the branch is pushed but no PR is open; see [ROADMAP.md](docs/05-development/ROADMAP.md) and the packet for exactly what remains.**
...
- **Contextual `?` help** added to five existing panels (sealed-realm status, airgap banner, capability self-check, vault status, provenance panel), each linking to a specific compiled glossary/guide entry where one exists, or to the documented "no content yet" fallback where it honestly doesn't (the capability panel, pending the P0.1–P0.16 backfill below).
- `docs/00-overview/glossary.md` fully rewritten with three-depth content for 42 of 46 entries (the 4 "Things people get wrong" corrections are deliberately kept single-depth, since they're the same warning at every reading level, not a concept needing to be explained three ways).
- `docs/03-guides/first-wallet.md` and `docs/03-guides/verify-a-hardware-wallet.md` gained three-depth content on their key explanatory passages, as a proven reference for the remaining 7 guides.
- New `test/help-content.test.js` (20 tests): the parser/renderer in isolation (shared-vs-group parsing, missing/partial-depth warnings, unterminated/duplicate-block failures, markdown-to-HTML escaping and rendering), plus end-to-end checks that the real `docs/` tree compiles and reaches the built artifact byte-for-byte.
- `scripts/run-browser-harness.js` gained `verifyHelpFramework` (depth switching with real content-change assertions, persistence across reload, offline search with a network-request tripwire, contextual-help navigation including the missing-topic fallback, inline glossary tap-to-define) — **written but not executed**; see below.
- **Bundle impact:** the compiled help content added roughly 350 KB to `build/coldbox.html` (from ~830 KB to ~1.01 MB) — well over the ~180 KB figure in `SPEC.md`'s bundle table, which predates this item and needs updating either way. Most of the weight is the existing `jsonScriptLiteral()` helper's `<`/`>`/`&` → `\uXXXX` escaping (shared with `PROVENANCE_LIBRARIES` and the cold-realm document, not new to this item) multiplying every HTML tag in the compiled content; an earlier draft also duplicated a full plain-text copy of every depth into a separate search-index field; removing that redundant copy and deriving search text from the already-embedded HTML at runtime instead cut the added weight by roughly a third. The remaining overage is flagged as a real, unresolved finding — see the packet's "what to scrutinise."
- **Known limitation, disclosed here and in the packet:** `npm run test:browser` could not be run in the authoring sandbox — outbound Playwright browser-binary downloads are blocked by that sandbox's network allowlist (same class of gap recorded on P0.16). `verifyHelpFramework` is implemented and manually reviewed against the actual DOM ids/classes it targets, but its real pass/fail in Chromium and Firefox is unverified.
