...
- P1.4a: give the derivation engine the two user-facing destinations it never had. **Derivation paths** reports the selected chain's SLIP-44 coin type, default account path, address encoding and the independent test vectors this build carries for it, and validates an arbitrary BIP-32 path before deriving that node's public metadata in Tier 3 generic mode. **Address derivation** derives a range of receive and change addresses for the focused released secret, each shown with its full path and encoding. Both are lenses on the focused secret under ADR-0045 — neither accepts a phrase, and both clear with the registry — and both call the existing P1.4/P1.5 engine rather than adding a second derivation path. The engine now exposes the account-path and coin-type facts the surfaces display, so those tables are not copied into the UI where they could drift. No extended private key, raw private key, WIF or phrase is rendered, logged or offered to a clipboard, and nothing derived crosses the realm boundary. A chain with no independently recorded test vectors cannot be selected on either surface, so a future custom registry entry stays underivable until its vectors exist. The sealed rail, mobile More sheet, sealed hub and the All flows index stop marking the two destinations unavailable; their rail and More-sheet entries carry the handled navigation attribute, because a bare fragment href inside the sandboxed frame resolves against the parent's `file://` URL and goes nowhere.
- UI.10b (passes 1 and 3 of 3, in progress): navigate the maintainer-approved workstation hierarchy in both realms. The warm rail becomes Workspace / Records / Trust & reference / Vault & settings / Sealed work and the sealed rail becomes Seeds & lineage / Forge / Derive / Split & carry / Recover & verify / Session, asserted against the approved manifest rather than a second hand-written list. The phone bar becomes the approved five object slots and both More sheets are realm-pure, so a sealed capability is never reached through a warm destination. Adds Wallets, Security & verify, Settings and an All flows index — the index is the reachability audit that lets the four tool-first placeholder pages be deleted instead of left as orphaned routes. Reconciles the committed browser harness to the new shell: destinations now carry stable `data-nav` handles, the approved sub-destinations deep link to the panel they name rather than three rail entries behaving as one, and Verify Bench and QR Studio are reached the way a user reaches them — through Security & verify and Wallets. Screen content and pixel convergence are later passes; nothing here claims parity.
- Roadmap: add **P1.4a Derivation paths and address derivation surfaces**. P1.4 and P1.5 completed the derivation engine, but no item ever owned its user-facing surfaces, so the shell had been marking those two destinations unavailable against two completed items — a claim that was not true. Also corrects two long-standing mislabels: Child seeds was owned by P1.5 (it is P4.6) and the recovery assistant named a bare P4.3 that the roadmap splits into P4.3a..P4.3e.
- UI.10a: import the maintainer-approved **Self-Custody Security Workstation** desktop and mobile mocks as new immutable references (approved 2026-08-19) and move the approved package from one reference set to two. The toolkit references frozen at UI.4a stay byte-identical as audit evidence and are binding on nothing; manifest schema v2 names exactly one set current and fails closed on an ambiguous, retired, shared or undeclared selection. `scripts/ui-reference-manifest.js` becomes the single place that answers which set is current, and corrects the prototype's own roadmap tags against `ROADMAP.md` under PAR-005 rather than trusting the artifact. Production `src/` is untouched: this item imports and selects, it does not certify. UI.10b implements the shell; UI.11 proves the pixels.
- Planning refinement: make Tor an explicit v1 Bitcoin-wallet transport policy without pretending browser JavaScript can create or prove Tor transport. WAL.2 distinguishes ordinary standard networking, an environment-provided Tor mode for Tor Browser/Tails or another separately configured Tor transport, and a Tor-enforced mode that requires a CSP-pinned v3 `.onion` source and fails closed with no clearnet fallback. WAL.10 carries the selected policy through exact-byte broadcast and monitoring; WAL.15 certifies ordinary-browser non-claims, Tor Browser/Tails behavior, onion failure without Tor, and the continued ban on broad `*.onion` CSP egress. This is future planning, not a claim that Tor-mode wallet networking ships today.
- Planning: pause additional UI.11 pixel-convergence work while the product identity is reorganized from a tool-first companion into **Coldbox — Self-Custody Security Workstation**. UI.10a delegates the actual replacement desktop/mobile mock design to a maintainer-approved design pass, then imports only explicitly approved artifacts as new immutable references; UI.10b implements that approved object/workflow-centered shell. The existing comic aesthetic, two-realm security boundary, specialist-tool coverage and truthful unavailable-feature behavior remain mandatory. Old approved references stay immutable evidence. This documentation change does not create or commit any mockup or external design handoff.
...
- Planning refinement: add a pre-v1 seed lineage and public identity campaign between SEC and WAL. It makes BIP-85 roots/children first-class linked records; keeps non-secret identification metadata readable while secrets stay sealed; lets each child choose external-only, separately stored Level 3, or explicitly authorized just-in-time root-derived Coldbox signing; exposes Standard/Compact SeedQR for eligible root/child seeds only inside the sealed realm; requires child/account xpub and descriptor export with privacy warnings; gives public addresses structured purpose/role/lineage/device/account identity instead of making free-form notes carry machine-relevant meaning; and adds a pre-v1 vault-credential floor plus sealed CSPRNG password/passphrase generator with a strength slider. WAL.13 separately guarantees unsigned, partially signed and finalized PSBT export before v1. This is planning, not a claim that the complete workflow already ships.
...
- UI.7 adds typed Send to routes from public records: addresses can open the
...
### Changed - UI.4 sealed-realm tool hub (2026-08-15)
...
- Added [ADR-0045](docs/05-development/adr/0045-released-secret-model.md): a secret is
  ...
  sealed frame; every other cold tool becomes a lens on the focused secret and loses
  only its duplicate seed/source loader. The five duplicate loaders are removed by
  UI.4; every input that does a different job stays — share and recovery entry, the
  separate BIP-39 passphrase, vault authentication and re-authentication, physical/manual
  entropy, and secret notes. Amends ADR-0023 and ADR-0028, which keep their contracts.
