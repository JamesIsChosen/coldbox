...
- **F3 (blocking, partially open).** `verifyProvenancePanel` in `scripts/run-browser-harness.js` gained assertions for F1's visible value and F2's hash-field-tamper case. Chromium/Firefox execution itself remains blocked in every sandboxed session so far — `cdn.playwright.dev` is outside the reachable network allowlist. `npm run test:browser` fails only with the pre-flight "Playwright browser binaries are missing" error, not a code/logic error. Needs a network-capable environment to actually execute.
