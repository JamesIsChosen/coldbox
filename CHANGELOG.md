...
- **Bundle size delta recorded against [SPEC §3](docs/01-spec/SPEC.md)'s budget (target ≤ 3 MB, hard cap 4.5 MB):** `build/coldbox.html` grows from 1,040,074 bytes (a clean build of `main` immediately before this branch) to 1,080,408 bytes, **+40,334 bytes (≈ +39.4 KB)** — the embedded licence text and its JSON-escaping overhead (≈37.7 KB) plus the new three-depth glossary entry backing the panel's contextual help (≈1.7 KB), in line with [ADR-0018](docs/05-development/adr/0018-agplv3-license.md)'s ≈34 KB estimate for the licence text itself. New total is ≈ 1.03 MB, well inside budget.
