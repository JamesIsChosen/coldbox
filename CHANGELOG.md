...
### Added — P0.17 help framework (2026-08-07)
...
- **`docs/00-overview/glossary.md` fully backfilled to three depths — all 46 entries**, including the four "Things people get wrong" corrections (initially left single-depth as a judgment call, then wrapped too since it cost little and closes the gap entirely) and five new entries backing previously-undocumented P0.1–P0.16 in-app copy: capability self-check, KDF profiles (Fast/Standard/Paranoid), save integrity, keyfile unlock, and the provenance panel itself.
...
- `scripts/run-browser-harness.js` gained `verifyHelpFramework` (depth switching with real content-change assertions, persistence across reload, offline search with a network-request tripwire, contextual-help navigation, a genuine fail-closed fallback case against a synthetic nonexistent topic, inline glossary tap-to-define) — **written but not executed**; see below.
...
- **Known limitation, disclosed here and in the packet:** `npm run test:browser` could not be run in the authoring sandbox — outbound Playwright browser-binary downloads are blocked by that sandbox's network allowlist (same class of gap recorded on P0.16). `verifyHelpFramework` is implemented and manually reviewed against the actual DOM ids/classes it targets — this review process caught and fixed one real bug (an assertion checking for text that didn't match the actual authored glossary prose) — but its real pass/fail in Chromium and Firefox is unverified. This is the reason the roadmap item stays at `[~]` rather than being handed to review as fully green.
