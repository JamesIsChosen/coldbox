...
- UI.11: add the manifest-driven Chromium/Firefox parity harness, exact PNG comparison artifacts, state-matrix ownership checks, shared warm/sealed font faces for chrome, mock-aligned masthead actions, sealed viewport sizing without an outer scrollbar, a cold-local fragment router for functional sealed navigation clicks, and continued desktop/mobile sealed-shell visual corrections. Certification remains open at 138 captured rows; the latest exact run reports 63,499,266 unexpected pixels and no physical mobile evidence.
