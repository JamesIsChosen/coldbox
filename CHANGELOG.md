...
- UI.11 (mid-item, harness pass): rewrite `scripts/ui11-parity.js` against the UI.10a approved-reference module instead of its own copies of `SCREEN_OWNERS`, the roadmap parser and the state matrix. The harness now reads the current set, viewports, comparison regions, screens and deviation register from one place; derives every reference navigation route from the approved artifact's own tables and cross-checks them against the manifest before it clicks anything; crops the mobile capture by measuring the approved product frame rather than hard-coding the superseded artifact's offsets; refuses pixel masks and percentage thresholds at run time; and refuses to skip a row. A deviation whose normalizer is not implemented blocks its rows as PENDING and fails the run, so a permitted difference is never confused with a removed one. Only PAR-008 is implemented; the other five are declared with reasons and are blocking. The reference half is exercised over all 92 states (88 captured, four entry points unresolved); no product comparison is attempted, because UI.10a and UI.10b are both still open and `ui-parity.md` §6.1 pauses convergence. Production `src/` is untouched and the build hash is unchanged.
