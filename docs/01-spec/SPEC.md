...
**Cold realm** — a sandboxed iframe: `<iframe sandbox="allow-scripts allow-downloads" srcdoc="…">`, carrying its own CSP with `default-src 'none'; connect-src 'none'`. Contains vault decryption, seeds, private keys, all derivation, SLIP-39/Shamir, BIP-85, recovery search, and secret QR generation.
...
The app detects available save mechanisms at boot. **Save vault** is the primary post-create action; lower-level File System Access/download/manual choices remain available and their verification limitations stay explicit. A verified File System Access save is labeled **Saved · verified**. Download/manual handoff is labeled **Saved · unverified** rather than falsely remaining **Not saved**, but the normal-lock confirmation remains active until that copy is reopened or a verified save succeeds. Saving never sends plaintext to the warm shell: the cold realm emits only encrypted `.cbx` bytes, and save/re-save must preserve the same authenticated Vault ID while advancing only that vault's generation. A normal user-requested lock — including the visible cold-realm **Request lock** control — routes through the warm shell and warns when the current vault is unsaved or unverified, offering save-first, lock-without-saving, or cancel. Panic hide, idle timeout, network-mode changes, and security-health failures lock immediately and never wait for storage.
