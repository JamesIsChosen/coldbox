...
:::
::: working
P1.9 provides five checks: device fingerprint, receive address, account xpub, metal backup, and BIP-39 passphrase. A match means the public value agrees with Coldbox's calculation. It does not prove the hardware or its display is genuine, because this version has no hardware connection.
:::
::: technical
The workflows reuse the cold-only BIP-39 PBKDF2 and BIP-32 derivation engines. Fingerprints compare eight hexadecimal characters; receive-address derivation uses an account-level Bitcoin xpub and a full address; Bech32 is case-insensitive while Base58 values remain exact. No verification input or result is sent over the warm/cold message channel.
:::
...
Verify Bench → Receive address in the sealed workspace. Paste the account xpub, select the script type matching your path (`84'` → Native SegWit, `86'` → Taproot), choose the receive/change chain and index, and derive one address.
...
2. In Coldbox, **offline**, open Verify Bench → Device fingerprint in the sealed workspace and enter the seed phrase you believe it holds.
3. Compare.
...
2. Verify Bench → Metal backup → enter the seed from your metal plate, exactly as engraved.
...
2. Offline in Coldbox, open Verify Bench → BIP-39 passphrase, enter the seed and the passphrase twice, and compare fingerprints.
...
Open Verify Bench → Account xpub. Enter the seed and optional passphrase, select the account's network, script type, and account number, then compare the complete derived xpub with the value exported from the device or held by your software. Long strings must be compared character by character; a prefix/suffix check is not a verification.
