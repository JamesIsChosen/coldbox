...
This branch implements [UI.3](../ROADMAP.md) as a cold-realm, session-only released-secret registry and switcher. Seed Forge can release several labeled results, exactly one remains focused, and the dependent split, backup-share, SeedQR, and verification panels visibly identify that focus by public master fingerprint. Teardown, clear-shortcut, and focus changes clear or invalidate cold-local derived state without adding a message type or persistence path.

## 2. Scope

In scope: the registry and zeroization path in `src/cold/main.js`; the cold switcher, Seed Forge release controls, focus indicators, and panel wiring in the cold HTML/CSS; Chromium/Firefox browser coverage; unit/static coverage; the UI.3 guide/glossary help; changelog and roadmap status.
...
The working tree was on `ui.3-released-secret-state`.
...
> npm run check-docs
Documentation hygiene check passed: 226 markdown file(s) checked, 0 warning(s).

> npm run build
Built build/coldbox.html (b214401be55a415f003d70fcfe00fa23f687b6fa3840ea5d19a489d2c527eb31)

> first clean build
bytes=2652783
sha256=b214401be55a415f003d70fcfe00fa23f687b6fa3840ea5d19a489d2c527eb31
...
The browser harness ran the built artifact from `file://` in both engines. Its dependency-free rebuild also matched byte-for-byte: `aa618188a33bac1f197479bf0d332a4bee95bb9e6886e5ff4cdb6841d4208f4e` in the harness's temporary verification copy.
...
| No released secret or derivative appears in a warm message; no persistence is added. | The registry factory contains no `postMessage`, storage, or vault-compartment operation. No protocol message or schema was added; release/focus stays in the cold closure and only existing public verification/dirty/status acknowledgements can cross the channel. | Unit/static boundary assertions; full protocol test suite; browser warm-shell text checks after two releases. |
...
The recorded UI.2 baseline is 2,622,481 bytes. This branch builds to 2,652,783 bytes, a delta of +30,302 bytes (+29.59 KiB, +1.16%); the artifact remains below the 4 MB target and 4.5 MB hard cap recorded in [dependencies.md](../dependencies.md#bundle-budget). The increase includes the UI.3 cold registry/switcher, browser assertions, automatic verification relinking, and the three-depth help additions.
