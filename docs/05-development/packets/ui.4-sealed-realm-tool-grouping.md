...
- The ADR-0045 declared secret-input registry and its static/dynamic ID test.
...
The UI.4 tests assert the exact six group order, hub targets, exact cold CSP policy, absence of all five retired IDs, and registry completeness. The address test preserves the pre-release Seed Forge verification path while released sessions remain comparison-only.
...
```text
$ npm run build
Built build/coldbox.html (bbad99e2e28e053f64f0d072dd0c3f6cc25389c6d9a3ca5d30abd5a19468c330)

$ npm run build
Built build/coldbox.html (bbad99e2e28e053f64f0d072dd0c3f6cc25389c6d9a3ca5d30abd5a19468c330)

$ Get-FileHash build/coldbox.html -Algorithm SHA256
BBAD99E2E28E053F64F0D072DD0C3F6CC25389C6D9A3CA5D30ABD5A19468C330
...
Playwright is dev-only; dependency-free build matches byte-for-byte (bbad99e2e28e053f64f0d072dd0c3f6cc25389c6d9a3ca5d30abd5a19468c330)
...
| Declared secret-input registry; exactly one `seed-entry` | `COLD_SECRET_INPUT_REGISTRY` declares static IDs and the dynamic Seed Forge, Seed XOR, Shamir39, and raw SSS IDs. The test expands the registry, rejects undeclared current inputs, rejects duplicate declarations, checks dynamic assignment prefixes, and asserts exactly one `seed-entry` record. | `test/ui.4-sealed-realm.test.js` |
...
If this implementation were wrong, the most serious plausible failure would be a migrated panel displaying or generating from an unfocused/stale secret, or a newly introduced undeclared secret input bypassing the review registry. The source registry and focus/teardown browser assertions are intended to make those failures visible, but dynamic DOM changes remain the least mechanically provable part of the check and are called out below.

## 6. Test evidence

New or changed coverage:

- `test/ui.4-sealed-realm.test.js` checks the registry, dynamic IDs, exact one `seed-entry`, removed fields, six groups, hub links, and exact cold CSP.
...
Deliberate negative checks include undeclared/duplicate registry entries, retired IDs still present, no focused secret, incomplete share sets, invalid checksums, malformed/mixed shares, confirmation-gated codex32 correction, wrong external public values, cold network probes, CSP tampering, and lock/teardown while secret-bearing workflows are populated. The full Node suite reports 408 passing tests, and the browser harness passes in both required engines.
...
- Review the registry against every current static and dynamically-created secret input, especially the dynamic per-word and share-combine IDs. A future input type or dynamic ID must update both the registry and its test model.
...
- The static registry test enumerates the current secret-bearing HTML input classes and explicit text fields, and checks the known dynamic prefixes. It is intentionally conservative about future DOM construction: a new dynamic input should be accompanied by a registry/test update rather than silently inferred.
...
The UI.3 packet recorded a local baseline of 2,654,759 bytes. This UI.4 branch builds to 2,660,608 bytes, a delta of +5,849 bytes (+5.71 KiB, +0.22%). The artifact remains below the 4 MB target and 4.5 MB hard cap recorded in [dependencies.md](../dependencies.md#bundle-budget).
