...
**ADR-0044 loosens a security-motivated rule.** It is the one change in this PR that could make the product worse. The failure mode is not a vulnerability but a credibility loss: if the panel/realm boundary is drawn wrongly by an implementer, a panel that asserts something about security could animate while making that assertion, at the moment a user is deciding whether to believe it. I mitigated this three ways — a permanence carve-out that did not exist before (the airgap guard, capability check, unlock and panic screens are now calm *unconditionally*, where previously they were calm only by virtue of a realm rule that was about to be relaxed); an explicit same-frame requirement so calm is a state and not an animation; and a stated tiebreaker that defaults to calm. The forbidden column is unchanged in strength. I still consider this the highest-risk item in the PR and it should get the most reviewer attention.
