...
5. At least one canonical `.cbx` save path works end-to-end on that platform; File System Access verifies read-back where available, the filename is `<name>--<id8>.cbx` with no visible generation, and an unchanged saved vault cannot create another look-alike copy
