...
**Cold realm** — a sandboxed iframe (`sandbox="allow-scripts allow-downloads allow-modals"`, `srcdoc`) with its own CSP setting `connect-src 'none'`. Owns vault crypto, passphrase entry, seeds, private keys, all derivation, and every Shamir scheme. The modal permission exists for cold-only printing; `allow-same-origin` remains absent.
