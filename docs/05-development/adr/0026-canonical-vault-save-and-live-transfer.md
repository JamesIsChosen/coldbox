...
1. **One Vault ID has one canonical local filename.** Current Coldbox names it `<public-name>--<id8>.cbx`. No current filename carries a user-visible save-generation suffix. **Amended by [ADR-0046](0046-vault-name-availability-at-unlock.md):** the canonical name becomes `coldbox--<id8>.cbx`, carrying no user-chosen text. One-Vault-ID-one-file and the absence of a generation suffix are unchanged; only the user-chosen portion is removed.
...
3. **Public vault names are unique within what Coldbox can actually know.** A different Vault ID cannot claim a name already owned in the current session, the best-effort browser-profile name registry, or the currently user-granted Vault Library. This is not a claim of disk-wide uniqueness: a local HTML app cannot silently enumerate the filesystem.
   - **Retired by [ADR-0046](0046-vault-name-availability-at-unlock.md), not reinterpreted.** There is no longer a public vault name to make unique. Names are typed in the sealed realm and stored inside each vault's own encrypted compartment, so **no party — Coldbox, the session, or the browser profile — can compare two vaults' names at all**, and the session/browser-profile/library scopes named above no longer exist as name scopes. The requirement is withdrawn rather than weakened: it existed to prevent look-alike confusion between vaults, and the discriminator visible at selection time is now `id8`, which derives from the authenticated Vault ID rather than from user-chosen text. A name nobody can see cannot be used to confuse anyone. The **best-effort browser-profile name registry** described above is withdrawn with the clause it served; its residual limit statement in *Negative / limits* below is retired on the same grounds. This is the clause referred to elsewhere in this repository as ADR-0026's duplicate-name refusal, restated in *Positive* below and retired there too.
4. **Vault ID remains the authority.** The cold-generated authenticated UUID is immutable. The public name and `id8` filename suffix are convenience metadata and are checked against the authenticated ID after unlock. **Amended by [ADR-0046](0046-vault-name-availability-at-unlock.md):** the canonical filename becomes `coldbox--<id8>.cbx` and carries no public name; `id8` alone remains the advisory suffix, still checked against the authenticated ID after unlock. Vault ID as the authority is unchanged.
...
7. **Sender requires an already-unlocked, durably backed named vault.** The vault must have been loaded from a durable `.cbx` or completed a verified canonical File System Access save; live QR cannot be the sender's first/only persistence path. **Amended by [ADR-0046](0046-vault-name-availability-at-unlock.md):** read the requirement as *already-unlocked and durably backed*. The word **named** is retired here — warm gates this action and warm never sees the vault's name, so a name was never something this gate could test. The durability condition, which is the load-bearing half, is unchanged. Warm requests a fresh encrypted `.cbx` representation from cold and displays only those encrypted bytes as a repeating QR stream. No passphrase, seed, plaintext secret compartment, or unlock authority is encoded.
...
- A second vault cannot silently reuse an already-known public name. **Retired by [ADR-0046](0046-vault-name-availability-at-unlock.md):** once names live inside their own encrypted vaults, no party can compare them, and the requirement is not needed — it existed to prevent look-alike confusion, and the discriminator visible at selection time becomes `id8`, which derives from the authenticated Vault ID rather than from user-chosen text. A name nobody can see cannot be used to confuse anyone.
...
- Browser-profile name uniqueness is best effort and cannot discover an unseen duplicate elsewhere on disk. **Retired by [ADR-0046](0046-vault-name-availability-at-unlock.md) together with §3, the requirement it qualifies.** Once names live inside their own encrypted vaults there is no browser-profile name registry and no name-uniqueness requirement, so there is no best-effort limit left to state — the limitation is not merely reduced, the thing it limited is gone. What replaces it is a different and honestly weaker property, recorded in ADR-0046 rather than here: **pre-unlock legibility**. A picker that can only show `id8` and an optional device-local nickname makes selecting or overwriting the wrong vault easier, which is an integrity and availability cost, not a disclosure one.
