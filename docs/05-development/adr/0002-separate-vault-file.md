...
- Saving from `file://` is awkward on iOS, requiring three fallback paths (§8.5).
...
Vault saving needs three paths because blob downloads from `file://` are unreliable on iOS Safari: File System Access API, blob download, and manual base64/QR export. The manual path is built as a first-class flow, not a fallback — for a phone-primary user it may be the normal one.
