...
- [~] **UI.2 Brand assets — wordmark and favicons**
  ...
  **Open gap (2026-08-15, author):** every acceptance criterion above is met and evidenced in [the packet](packets/ui.2-brand-assets.md), but the item does **not** yet satisfy AGENTS.md §5's "one desktop and one mobile browser from `file://`" clause. Desktop Chromium from `file://` is verified; Firefox could not be installed in the authoring environment, and physical mobile is untested. [ADR-0043](adr/0043-scoped-mobile-validation-deferral.md) is scoped to P2.7 by its own terms and is **not** borrowed here. Closing this needs either a physical mobile result or an explicit maintainer deferral recorded as its own decision.
