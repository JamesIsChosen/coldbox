...
- [~] P1.9 Verification workflows: fingerprint, receive address, xpub, backup, passphrase ⚠️ *implementable by agent; final validation needs real hardware wallets*
  **Current state (recorded 2026-08-10):** five cold-local verification workflows accept manual device/public values, derive with the existing BIP-39/BIP-32 engine, compare complete public results, clear secret inputs after each attempt and on lock, and disclose that no hardware transport or authenticity claim exists. Evidence is in `packets/p1.9-verification-workflows.md`; the marker remains `[~]` for independent review and the real-device validation gate remains open.
