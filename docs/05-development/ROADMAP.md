...
- [~] P1.10 QR generation: addresses, SeedQR, Compact SeedQR, printable cards
  **Current state (recorded 2026-08-10):** public address QR generation supports local BIP-21/EIP-681 payloads with SVG/PNG export in the warm shell; standard and Compact SeedQR generation, explicit plaintext acknowledgement, printable layouts, transcription grid, and SVG/PNG export remain cold-only. Evidence is in `packets/p1.10-qr-generation.md`; the marker remains `[~]` for independent review.
