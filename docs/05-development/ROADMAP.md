...
- [~] **P1.4a Derivation paths and address derivation surfaces**
  *Deps: P1.4, P1.5*
  P1.4 and P1.5 completed the derivation **engine**. No item ever owned the user-facing surfaces for it, so the engine is reachable today only indirectly, through Verify Bench's fingerprint/address/xpub comparison. The approved workstation design (UI.10a) gives **Derivation paths** and **Address derivation** their own sealed-realm destinations, and until this item ships the shell has no truthful owner to name for them — marking them unavailable against the completed P1.4/P1.5 asserts something false, and marking them available promises a surface that does not exist. Raised during UI.10b, which could not satisfy its own truthful-availability criterion without it.
  **Accept:** both destinations exist inside the sealed realm and are reachable from the approved rail, the mobile More sheet and the All flows index; **Derivation paths** lets a user choose a chain, see its coin type, default account path and address encoding, and enter an arbitrary path in the P1.5 generic mode, with the path validated before use; **Address derivation** derives a range of receive and change addresses for the focused released secret and shows each with its full path and encoding; both read the focused released secret and never accept a phrase in their own fields (ADR-0045); no derived private key, xprv or phrase is displayed, logged or placed on a clipboard, asserted by a test; results are computed cold-locally with no message crossing the realm boundary; a custom chain registry entry cannot be used until independent test vectors are recorded for it; the surfaces reuse the existing `src/cold/derivation.js` engine rather than adding a second derivation path; keyboard access, focus return and 44px touch targets meet the shell floors; the desktop and mobile presentation matches the UI.10a approved reference for these destinations under the [parity contract](../01-spec/ui-parity.md).
  **Out of scope:** descriptors (P4.9), child seeds (P4.6), and any wallet-scoped address lifecycle, which is WAL.4.
  **Current state (recorded 2026-08-20):** both destinations are built in the sealed **Derive** workspace as lenses on the focused released secret, reusing the P1.4/P1.5 engine; the sealed rail, mobile More sheet, sealed hub and the All flows index reach them and no longer mark them unavailable. Evidence is in `packets/p1.4a-derivation-surfaces.md`. The branch is stacked on `ui.10b-workstation-shell` because the approved rail, mobile More sheet and All flows index this item's acceptance names are UI.10b deliverables; it cannot merge before UI.10b passes. The parity criterion is recorded as **not met** in the packet: pixel convergence against the approved references is paused under [ui-parity.md §6.1](../01-spec/ui-parity.md#61-product-identity-redesign-before-ui11-resumes) until UI.10b and UI.11 close, so these two destinations inherit the contract but cannot be certified against it yet. The marker stays `[~]` for independent review.
  **Review outcome (recorded 2026-08-20):** independent review of `30a9b1d` returned **FAIL** with three findings; the report is `packets/p1.4a-derivation-surfaces.review.md`. **F1** — the parity criterion is unmet, and the two screens are now carried in the [certification debt register](../01-spec/ui-parity.md#45-certification-debt-register) so the uncompared `PARITY` rows stay visible until UI.11 executes the comparison. **F2** — the custom-chain vector gate accepts a prose label; the real gate is specified in [chain-registry.md](../01-spec/chain-registry.md#what-has-test-vectors-has-to-mean-in-code) and owned by **P1.4b**, which **P4.2** now depends on, so no custom chain can ship while the gate is fake. **F3** — the branch is stacked on `ui.10b-workstation-shell`, which has not passed review; UI.10b is reviewed first and P1.4a is then reconciled onto the approved result and re-reviewed at a fresh head. Deferring F1 and F2 keeps two acceptance criteria unmet, so this item cannot reach `[x]` on the strength of the deferral alone — whether it closes with them carried is a maintainer decision recorded at re-review, not a reviewer reinterpretation.
- [ ] **P1.4b Executed independent-vector gate for the chain catalogue**
  *Deps: P1.4a*
  P1.4a shipped a gate that decides a chain is vector-backed when its `testVectors` field is a non-empty **string**, so a chain becomes selectable by carrying a plausible label. The P1.4a review recorded this as finding F2 against `chainHasIndependentVectors` in `src/cold/main.js`. Nothing is exploitable in that build — the catalogue is frozen at build time and no surface can add a chain — but the mechanism that is supposed to stop an unproven chain from being derived from does not do it, and P4.2 is the item that makes that reachable. Requirements are in [chain-registry.md](../01-spec/chain-registry.md#what-has-test-vectors-has-to-mean-in-code).
  **Accept:** each shipped chain carries vector **records** with `source`, `seed`, `path` and `expectedAddress` rather than a prose string, and a record missing any field is invalid; every chain offers at least three distinct paths, and records sharing one path count once; the gate derives from `seed` along `path` with the shipped `src/cold/derivation.js` engine and compares against `expectedAddress`, so a chain is selectable only when every one of its vectors executed and matched in this build; absence, malformation and mismatch each make the chain unusable rather than usable-with-a-warning; CI executes the same vectors and fails non-zero on any absence, malformation or mismatch; a negative test proves a chain entry carrying only a plausible label stays unselectable, and the positive test asserts the vectors actually ran rather than asserting selectability alone.
  **Out of scope:** adding chains (P4.1) and the user-facing custom registry itself (P4.2).
...
**The handoff's implementation order from UI.3 through UI.8 is load-bearing.** The released-secret state has to exist before anything can be tested against it; the phrase fields cannot be deleted until it does; the floating menu is built once because forty-odd surfaces use it. UI.4a freezes the parity contract. On 2026-08-17 the maintainer approved a product-identity redesign before certification: UI.10a owns the replacement approved mock design/import, UI.10b implements that approved workstation shell, and UI.11 pixel convergence is paused until both are `[x]`. UI.11 remains the hard final parity gate before Phase 2 resumes.
...
- [~] **UI.10a Product identity and replacement approved mock design**
  *Deps: UI.8, UI.9, UI.10*
  Execute [ADR-0059](adr/0059-self-custody-workstation-product-identity.md) and the explicit [ui-parity.md §6.1](../01-spec/ui-parity.md#61-product-identity-redesign-before-ui11-resumes) change control before any further UI.11 pixel convergence.
  **Accept:** Coldbox's durable product identity is reconciled as **Self-Custody Security Workstation**, with the finished v1 direction including a complete standalone single-signature Bitcoin wallet while current pre-release availability continues to come only from roadmap status; the replacement desktop/mobile design is produced for maintainer approval without altering production `src/`; the design remains recognizably the approved Coldbox comic aesthetic rather than a generic fintech/SaaS rebrand; information architecture is object/workflow-centered rather than tool-first while every current specialist capability remains mapped to a contextual or Advanced-tools destination; exact navigation/grouping is owned by the maintainer-approved replacement mocks rather than predetermined by roadmap prose; warm/cold authority, calm security panels, accessibility, 44px mobile targets, secret switcher/record identity, truthful unavailable future features, and offline/single-file constraints remain non-negotiable; the redesign accommodates the accepted SEED/WAL/SEC concepts including seed lineage, root/child SeedQR, structured address identity, vault credential generation, standalone Bitcoin wallet/signing and later optional hardware signers without presenting unfinished functionality as currently available; after maintainer approval, the new desktop/mobile artifacts are imported as **new immutable reference files** with new hashes/byte sizes/viewports/screen inventory/navigation metadata, the old approved files remain byte-identical audit evidence, and the manifest/harness unambiguously selects the new set as current; reference-integrity/docs tests pass. External design handoffs or working mock files are not repository artifacts unless and until the maintainer separately approves their import.

- [ ] **UI.10b Self-custody workstation shell and workflow implementation** 🌐
  *Deps: UI.10a*
  Implement the maintainer-approved replacement design before final parity resumes.
  **Accept:** the actual app's primary mental model is no longer a drawer of unrelated tools; users navigate their wallets/accounts/addresses, seeds/secrets, devices, backups/recovery, portfolio/records, security/verification, vault/settings and reference/help through the exact hierarchy approved in UI.10a; every previously reachable Entropy/Seed Forge/derivation/share/Codex32/QR/hash/verification/recovery and other specialist capability remains reachable with no orphaned route; warm destinations do not gain cold capability, cold/warm CSP and MessageChannel guarantees remain intact, and secret-capable actions stay cold-owned; future SEC/SEED/WAL/P3/P4/P5 destinations remain disabled/unavailable with roadmap ownership until implemented; no payment wall, login, activation, advertising, telemetry, DRM or donation prompt enters a high-stakes workflow; desktop/mobile navigation, focus return, keyboard access, touch targets, responsive behavior and all existing functional tests are reconciled to the new shell; full vendor/lint/docs/unit/browser/reproducibility validation passes.

- [ ] **UI.11 Approved desktop/mobile visual parity certification** 🌐
  *Deps: UI.8, UI.9, UI.10*
  Close the interface restructure only after UI.10a and UI.10b are `[x]` and the built application is proven against the maintainer-approved replacement desktop/mobile references. Further pixel-convergence work is paused while either redesign item remains open. This item may correct visual drift and maintain the dedicated comparison harness; it may not invent another product hierarchy or add unrelated product behaviour.
  **Accept:** UI.10a and UI.10b are `[x]`; the parity harness selects the current maintainer-approved replacement reference set and reads its viewports, comparison regions, navigation and screen lists directly from the approved manifest; superseded reference artifacts remain byte-identical historical evidence and cannot accidentally become current; every manifest state is classified exactly once under [ui-parity.md](../01-spec/ui-parity.md), with the shared shell and every already-built feature classified `PARITY`, later-roadmap screens classified `UNAVAILABLE`, and no missing or skipped row; an unavailable screen appears only as the approved disabled navigation treatment and cannot be focused or opened; reference normalizers are deterministic, each names one registered deviation ID, each fails on unexpected selector cardinality, and no pixel mask or percentage threshold exists; in each browser engine required by the committed harness, every `PARITY` row has equal capture dimensions and zero unexpected changed pixels at the manifest comparison region; the packet includes the generated state matrix, reference/product/diff artifacts and machine-readable totals, and identifies every applied deviation by ID; keyboard, focus return, pinch zoom, responsive overflow, minimum touch targets, reduced motion, calm-panel state and dark/light presentation pass their committed assertions; a maintainer compares the real build with both approved references and records the physical mobile device, OS, browser and orientation — an ADR-0043 deferral does **not** close this item; no new deviation is accepted inside this implementation item without prior maintainer approval and the change-control steps in the contract; cold/warm CSP hashes, realm isolation and all existing behaviour remain intact; the reference artifacts remain absent from the production HTML; and the final build is reproducible.
...
- [ ] **WAL.2 — Bitcoin data-source and source-assurance layer**
  ...
  **Accept:** a user-owned browser-compatible Esplora/electrs-style HTTP source is supported as the preferred private mode; any direct Bitcoin Core RPC path must separately prove safe browser/CORS/credential behavior before it is claimed; public-source and cross-check modes expose their privacy/trust tradeoff; transport policy is explicit and separate from source assurance: `standard` means ordinary browser networking with no Tor claim, `tor-environment` means Tor Browser/Tails or another separately configured Tor transport supplied by the execution environment and not fingerprinted/proven by Coldbox, and `tor-enforced-onion` requires a reviewed CSP-pinned v3 `.onion` source; selecting Tor-enforced onion mode while no usable Tor path exists fails sync closed and never falls back to a clearnet alias/provider; the product does not expose an in-page "enable Tor" claim for ordinary Edge/Brave/Chrome/Firefox; the shipped CSP keeps a finite exact host allowlist and must not add a broad `*.onion` wildcard, while a user-owned unshipped onion host requires an exact reproducibly customized CSP/build or separately reviewed local bridge; source identity/tip/staleness plus source-assurance and transport modes accompany sync results; no xpub is handed to a public third party merely to scan; contradictions in material spend state are surfaced and block spending rather than majority-voted silently; every new clearnet/onion host is documented in `api-sources.md`, CSP and provenance.
...
- [ ] **WAL.10 — Exact-byte broadcast, pending reservations and confirmation monitor**
  ...
  **Accept:** cold supplies finalized bytes plus expected transaction id; warm cannot rebuild them; returned/tracked txid must match; signed inputs become locally reserved before broadcast; broadcast and subsequent monitoring inherit WAL.2's selected transport policy, and `tor-enforced-onion` failure blocks rather than falling back to any clearnet source; a privacy-mode downgrade requires an explicit visible user action and is never an automatic error-recovery path; broadcast, mempool, confirmed, replaced, conflicted, dropped and reorganized are distinct; "submitted" is never displayed as "confirmed"; source disagreement cannot silently release a reserved input; restart/reload preserves authenticated pending state and its transport-policy context.
...
- [ ] **WAL.15 — v1 wallet adversarial and physical certification** 🌐
  ...
  **Accept:** deterministic fuzz/property/differential suites cover descriptors, raw transactions, PSBT, API/node responses, fee/selection, signing, RBF/CPFP and reorg/conflict state; supported transaction families match independent implementations; negative mutations never partially authorize/sign; full browser harness passes; wallet desktop/mobile approved states pass their parity/accessibility checks; transport certification proves ordinary Edge/Brave/Chrome/Firefox never claim Tor merely from a Coldbox preference, Tor Browser/Tails can exercise the environment-provided path, a pinned v3 onion source fails closed when Tor transport is unavailable, `tor-enforced-onion` sync/broadcast never emits a clearnet fallback request, and the production CSP contains no broad onion wildcard; mainnet-value signing is not required for automated tests, but a maintainer records safe physical-device end-to-end test-network/regtest evidence across the supported execution matrix including the supported Tor path; all current specs/threat/API/standards/guides are reconciled; the final reproducible artifact has no unexplained network host or signing capability.
...
- [ ] P4.1 Tier 1 remaining chains
- [ ] **P4.2 Custom coin registry**
  *Deps: P1.4b*
  User-defined chains per [chain-registry.md](../01-spec/chain-registry.md#custom-registry): name, coin type, curve, address scheme, version bytes or HRP, WIF version, exportable and importable as JSON. This is the item that makes an unproven chain reachable by a user, so it cannot start until P1.4b makes the independent-vector gate real; building it against the P1.4a gate would ship a check that a plausible label defeats.
  **Accept:** a custom entry is refused until its vectors execute and match under the P1.4b gate; custom chains are visibly marked **unverified** and never imply shipped-chain confidence; import rejects a definition whose vectors are absent, malformed or mismatched, and says which.
