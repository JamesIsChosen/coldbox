...
- [~] **P0.17 — Help framework**
  ...
  **Known gap, disclosed rather than hidden, across both review rounds:** neither round of fixes has been run end-to-end through an actual browser by the author — this authoring sandbox still cannot download Playwright's binaries (`cdn.playwright.dev` returns `403` under its network allowlist). Both reviewers independently confirmed, with real network access, that Chromium and Firefox install, launch, and can exercise this app directly — one reviewer's own ad hoc probe went as far as confirming depth-switching, persistence, and offline search work correctly against a real build — so there is real evidence the implementation works, but the repository's own `npm run test:browser` harness (which is what the 🌐 marker actually names) has never completed a full run against any revision of this branch. Left at `[~]` per review-protocol.md: an item whose 🌐-marked criterion can't be verified is never `[x]`, and only the independent reviewer can flip it once that run happens.
