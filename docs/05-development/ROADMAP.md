...
- [~] **P0.17 — Help framework**
  ...
  **Current state (recorded 2026-08-07, mid-item, no PR open — see the packet):** the compiler, the `::: plain/working/technical` block model, the Learn-page UI (depth switcher, offline search, contextual `?`, inline glossary), and their tests are implemented and passing. `docs/00-overview/glossary.md` is fully backfilled to three depths (42 of 46 entries; the 4 "Things people get wrong" corrections are deliberately single-depth — see the packet). Two guides (`first-wallet.md`, `verify-a-hardware-wallet.md`) have three-depth content on their key explanatory passages. **Not done:** the other 7 guides remain plain-only (each triggers the documented build warning, not a failure), and the P0.1–P0.16 in-app-copy backfill (capability panel, CSP/airgap banner, vault UI, KDF profiles, keyfile unlock) has not been started at all — the capability panel's contextual `?` intentionally points at a not-yet-compiled `guide:capability-self-check` topic to demonstrate the fail-closed fallback rather than a fabricated mapping. `npm run test:browser` could not be executed in the authoring sandbox (Playwright's Chromium/Firefox downloads are blocked by that sandbox's network allowlist, same class of gap recorded on P0.16); the new `verifyHelpFramework` harness function is written but unexecuted. Left at `[~]`, not handed to review, until the backfill obligation above is actually met — see the packet for the full plan.
