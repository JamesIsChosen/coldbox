...
- [ ] **UI.11 Approved desktop/mobile visual parity certification** 🌐
  *Deps: UI.8, UI.9, UI.10*
  **Current state (recorded 2026-08-20):** the dedicated comparison harness has been rebuilt on `ui.11-parity-harness` and reads the current approved set, state matrix, classifications and deviation register from [ui-reference-manifest.js](../../scripts/ui-reference-manifest.js). The reference half is exercised over all 92 states; the product half cannot run, because certification is gated on UI.10a and UI.10b being `[x]` and neither is. Only the PAR-008 normalizer is implemented; every other row is blocked as PENDING and the run exits non-zero. No pixel convergence has been attempted, per [ui-parity.md §6.1](../01-spec/ui-parity.md#61-product-identity-redesign-before-ui11-resumes). Evidence is in `packets/ui.11-approved-visual-parity-certification.md`; the marker stays `[ ]`.
