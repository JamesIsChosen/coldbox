...
- [~] **P0.17 — Help framework**
  ...
  **Current state (recorded 2026-08-07):** the compiler, the `::: plain/working/technical` block model, the Learn-page UI (depth switcher, offline search, contextual `?` on five panels, inline glossary), and their tests are implemented and passing. **The backfill obligation above is met**: every glossary term (46 of 46 — including the four "Things people get wrong" corrections) and all nine `docs/03-guides/` files now carry three-depth content, and `npm run build` produces **zero** help-content warnings. New glossary entries back the P0.1–P0.16 in-app copy that previously had none (capability self-check, KDF profiles, save integrity, keyfile unlock, the provenance panel itself), and every shipped contextual `?` button resolves to real content — none point at a placeholder. **Known gap, disclosed rather than hidden:** `npm run test:browser` could not be executed in the authoring sandbox — Playwright's Chromium/Firefox binary downloads are blocked by that sandbox's network allowlist (`cdn.playwright.dev` returns `403`), the same class of gap P0.16 recorded and for the identical reason. The new `verifyHelpFramework` harness function is written, covers depth-switch rendering with content-change assertions, persistence, offline-search network tripwire, contextual help (including a genuine fail-closed fallback case), and inline glossary — but its real pass/fail in an actual browser is unverified. Left at `[~]` per review-protocol.md: an item whose 🌐-marked criterion can't be verified is never `[x]`, and only the independent reviewer can flip it once that verification happens (in this sandbox or a follow-up session with network access).
