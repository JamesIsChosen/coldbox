...
- [~] **P0.17 — Help framework**
  ...
  **Known gap, disclosed rather than hidden:** the F1 fix has not yet been re-verified end-to-end by actually running `npm run test:browser` to completion in Chromium and Firefox (this authoring sandbox still cannot download Playwright's browser binaries — `cdn.playwright.dev` returns `403` under its network allowlist). The new `verifyHelpFramework` harness function is written, covers depth-switch rendering with content-change assertions, persistence, offline-search network tripwire, contextual help (including a genuine fail-closed fallback case), and inline glossary — but with F1 fixed, whether it fully passes still needs a fresh run with real network access. Left at `[~]` per review-protocol.md: an item whose 🌐-marked criterion can't be verified is never `[x]`, and only the independent reviewer can flip it once that verification happens.
