...
7. **Inject provenance data** — the Reference → Provenance panel's build-time inputs are compiled in: the embedded library list (read directly from `vendor/vendor-manifest.json`, so it can't drift from what `verify-vendor` checks), and the build date (the commit date of the most recent commit touching `assets/`, `src/`, `scripts/`, or `vendor/` — deliberately *not* literal `HEAD`, so that a governance-only commit such as a PR packet update doesn't change the product's own bytes; see the [ADR-0015 amendment](adr/0015-provenance-build-date-and-self-hash.md#amendment-2026-08-06-build-date-is-scoped-to-product-paths-not-literal-head)).
