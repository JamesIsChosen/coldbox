...
Seed derivation uses the path `m / purpose' / coin_type' / account' / change / index`, with account and purpose levels hardened. A batch defaults to 20 addresses and is capped at 1,000; the engine refuses a batch that would enter the hardened index range. Watch-only derivation accepts non-hardened children below an account-level xpub/ypub/zpub/tpub/upub/vpub. An xpub cannot prove the master fingerprint, so watch-only results expose only the account fingerprint.
