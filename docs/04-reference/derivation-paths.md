...
The P1.4/P1.5 engine runs inside the sealed cold realm. Bitcoin accepts a copied raw seed of 16–64 bytes or an account-level extended public key and derives the four Tier 1 single-key script types. EVM uses `m/44'/60'/account'/change/index`, with one address covering all EVM chains; the generic mode accepts any canonical BIP-32 path. Public Bitcoin/EVM helpers return only public metadata. The explicit generic recovery projection can return raw private material, extended private keys, and compressed WIF, but only to cold-local callers; none is part of the public protocol result and no derivation operation needs network access.
