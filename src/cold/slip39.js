 ...
 * Coldbox replaces Node crypto with the bundled cold-realm primitives and
 * exposes only the explicit API below; no secret value crosses the frame.
 */
(function (global) {
  'use strict';

  var noble = global.__coldboxNobleCrypto;
  if (!noble || typeof noble.sha256 !== 'function' || typeof noble.pbkdf2 !== 'function') {
    throw new Error('SLIP-39 requires the bundled SHA-256 and PBKDF2 implementations.');
  }

  function asBytes(value) {
    if (value instanceof Uint8Array) return new Uint8Array(value);
    if (Array.isArray(value)) return new Uint8Array(value);
    throw new Error('SLIP-39 crypto input must be bytes.');
  }
  ...
  function hmacSha256(key, message) {
    ...
    var messageBytes = asBytes(message);
    if (keyBytes.length > blockLength) keyBytes = new Uint8Array(noble.sha256(keyBytes));
    var padded = new Uint8Array(blockLength);
    padded.set(keyBytes);
    var inner = new Uint8Array(blockLength + messageBytes.length);
    var outer = new Uint8Array(blockLength + 32);
    for (var index = 0; index < blockLength; index += 1) {
      inner[index] = padded[index] ^ 0x36;
      outer[index] = padded[index] ^ 0x5c;
    }
    inner.set(messageBytes, blockLength);
    var innerHash = new Uint8Array(noble.sha256(inner));
    outer.set(innerHash, blockLength);
    var result = new Uint8Array(noble.sha256(outer));
    keyBytes.fill(0); padded.fill(0); inner.fill(0); outer.fill(0); innerHash.fill(0);
    return result;
  }
  ...
  var crypto = {
    ...
    pbkdf2Sync: function (password, salt, iterations, length) {
      return noble.pbkdf2(noble.sha256, asBytes(password), asBytes(salt), { c: iterations, dkLen: length });
    },
    createHmac: function (_algorithm, key) {
      var chunks = [];
      return {
        update: function (value) { chunks.push(asBytes(value)); },
        digest: function () {
          var length = chunks.reduce(function (total, chunk) { return total + chunk.length; }, 0);
          var message = new Uint8Array(length);
          var offset = 0;
          chunks.forEach(function (chunk) { message.set(chunk, offset); offset += chunk.length; });
  ...
  function validateMasterSecret(value) {
    var secret = asBytes(value);
    try {
      if (secret.length < 16 || secret.length > 64 || secret.length % 2 !== 0) {
        throw new Error('SLIP-39 master secret must be 16 to 64 bytes and have an even length.');
      }
      return secret;
    } catch (error) {
      zeroBytes(secret);
  ...
  function validatePassphrase(value) {
    if (typeof value !== 'string') {
      throw new Error('SLIP-39 passphrase must be text.');
    }
    if (!/^[\x20-\x7e]*$/.test(value)) {
      throw new Error('SLIP-39 passphrase must contain printable ASCII characters only.');
    }
    return value;
  }
  ...
  function normalizeOptions(options) {
    var value = options || {};
    var groups = value.groups === undefined ? [{ threshold: 2, count: 3 }] : value.groups;
    if (!Array.isArray(groups) || groups.length < 1 || groups.length > 16) {
      throw new Error('SLIP-39 group count must be between 1 and 16.');
    }
    var groupThreshold = value.groupThreshold === undefined ? 1 : value.groupThreshold;
    if (!Number.isInteger(groupThreshold) || groupThreshold < 1 || groupThreshold > groups.length) {
      throw new Error('SLIP-39 group threshold is invalid.');
    }
    var normalizedGroups = groups.map(function (group) {
      if (!group || !Number.isInteger(group.threshold) || !Number.isInteger(group.count)
        || group.threshold < 1 || group.threshold > group.count || group.count > 16) {
        throw new Error('Each SLIP-39 group must have a valid member threshold and count.');
      }
      if (group.threshold === 1 && group.count > 1) {
        throw new Error('A member threshold of 1 only supports one share.');
      }
      return { threshold: group.threshold, count: group.count };
    });
    var identifier = value.identifier === undefined ? null : value.identifier;
    if (identifier !== null && (!Number.isInteger(identifier) || identifier < 0 || identifier >= 32768)) {
      throw new Error('SLIP-39 identifier must be a 15-bit integer.');
    }
    var iterationExponent = value.iterationExponent === undefined ? 0 : value.iterationExponent;
    if (!Number.isInteger(iterationExponent) || iterationExponent < 0 || iterationExponent > 15) {
      throw new Error('SLIP-39 iteration exponent must be between 0 and 15.');
    }
    var extendableBackupFlag = value.extendableBackupFlag === undefined ? 1 : value.extendableBackupFlag;
    if (extendableBackupFlag !== 0 && extendableBackupFlag !== 1) {
      throw new Error('SLIP-39 extendable backup flag must be 0 or 1.');
    }
    var passphrase = validatePassphrase(value.passphrase === undefined ? '' : value.passphrase);
    return { groups: normalizedGroups, groupThreshold: groupThreshold, identifier: identifier,
      iterationExponent: iterationExponent, extendableBackupFlag: extendableBackupFlag,
      passphrase: passphrase };
  }

  function zeroArray(value) {
    if (!Array.isArray(value)) return;
    for (var index = 0; index < value.length; index += 1) value[index] = 0;
  }

  function generate(masterSecret, options) {
    var normalized = normalizeOptions(options);
    var secret = validateMasterSecret(masterSecret);
    var randomIdentifier = null;
    var identifier = null;
    var identifierBytes = null;
    var encrypted = null;
    var groupShares = null;
    var shares = [];
    try {
      randomIdentifier = crypto.randomBytes(2);
      identifier = normalized.identifier === null
        ? (((randomIdentifier[0] << 8) | randomIdentifier[1]) & 0x7fff)
        : normalized.identifier;
      identifierBytes = intToIndices(BigInt(identifier), ITERATION_EXP_WORDS_LENGTH, 8);
      encrypted = crypt(Array.prototype.slice.call(secret), normalized.passphrase, normalized.iterationExponent,
        identifierBytes, normalized.extendableBackupFlag, true);
      groupShares = splitSecret(normalized.groupThreshold, normalized.groups.length, encrypted);
      normalized.groups.forEach(function (group, groupIndex) {
        var memberShares = null;
        try {
          memberShares = splitSecret(group.threshold, group.count, groupShares[groupIndex]);
          memberShares.forEach(function (share, memberIndex) {
            shares.push({
              mnemonic: encodeMnemonic(identifierBytes, normalized.extendableBackupFlag, normalized.iterationExponent,
                groupIndex, normalized.groupThreshold, normalized.groups.length, memberIndex, group.threshold, share),
              groupIndex: groupIndex, memberIndex: memberIndex, memberThreshold: group.threshold
            });
          });
        } finally {
          if (memberShares) memberShares.forEach(zeroArray);
        }
      });
      return Object.freeze({ identifier: identifier, extendableBackupFlag: normalized.extendableBackupFlag,
        iterationExponent: normalized.iterationExponent, groupThreshold: normalized.groupThreshold,
        groups: Object.freeze(normalized.groups), shares: Object.freeze(shares) });
    } finally {
      zeroBytes(secret);
      zeroArray(encrypted);
      if (groupShares) groupShares.forEach(zeroArray);
      zeroBytes(randomIdentifier);
    }
  }

  function recover(mnemonics, passphrase) {
    if (!Array.isArray(mnemonics) || mnemonics.length === 0) {
      throw new Error('At least one SLIP-39 share is required.');
    }
    var seenMembers = Object.create(null);
    mnemonics.forEach(function (mnemonic, index) {
      var decoded;
      try {
        decoded = decodeMnemonic(mnemonic);
      } catch (error) {
        throw new Error('SLIP-39 share ' + (index + 1) + ' is invalid: ' + error.message);
      }
      var key = decoded.identifier + ':' + decoded.groupIndex + ':' + decoded.memberIndex;
      if (seenMembers[key]) {
        throw new Error('SLIP-39 share ' + (index + 1) + ' repeats a member index.');
      }
      seenMembers[key] = true;
    });
    return combineMnemonics(mnemonics, validatePassphrase(passphrase === undefined ? '' : passphrase));
  }
...
function roundFunction(round, passphrase, exp, salt, secret) {
  ...
  const key = crypto.pbkdf2Sync(
    asBytes(roundedPhrase),
    asBytes(saltedSecret),
    count,
    secret.length,
    "sha256",
  );
...
function createDigest(randomData, sharedSecret) {
  const hmac = crypto.createHmac("sha256", asBytes(randomData));

  hmac.update(asBytes(sharedSecret));
...
// CommonJS exports removed for the browser bundle.

  var api = Object.freeze({
    constants: Object.freeze({ minSecretBytes: 16, maxSecretBytes: 64, wordCount128: 20, wordCount256: 33 }),
    wordList: Object.freeze(WORD_LIST.slice()),
    generate: generate,
    recover: recover,
    decode: decodeMnemonic,
    validate: validateMnemonic,
    bytesEqual: function (left, right) {
      if (!left || !right || left.length !== right.length) return false;
      var difference = 0; for (var index = 0; index < left.length; index += 1) difference |= left[index] ^ right[index];
      return difference === 0;
    }
  });

  Object.defineProperty(global, '__coldboxSlip39', { configurable: false, enumerable: false, value: api, writable: false });
}(window));
