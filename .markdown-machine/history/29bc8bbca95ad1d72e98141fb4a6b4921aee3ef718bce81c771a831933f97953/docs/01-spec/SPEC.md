...
> **[design-system.md](design-system.md) is authoritative for anything a user can see** — tokens, typography, components, the copy contract, and the accessibility floors. The approved desktop/mobile arrangement, permitted deviations and parity evidence are binding in [ui-parity.md](ui-parity.md). Together they supersede the visual direction originally given in this section. Rationale in [ADR-0009](../05-development/adr/0009-comic-visual-language.md) and [ADR-0049](../05-development/adr/0049-approved-mock-parity-contract.md). The rules below that are *not* purely visual — secret display, mobile, accessibility, onboarding — remain in force.
...
**A browser is not a secure element, and Coldbox does not pretend otherwise.** The current build does not construct, sign, or broadcast transactions — that capability is owned by the WAL phase and is unavailable until those items ship. What Coldbox does today is the layer your devices can't do for you: verifying that what your screen shows matches what your device actually holds, engineering backups that survive, tracking which of your devices holds which key, and making sure someone can find all of it when you're not around. The accepted v1 direction ([ADR-0051](../05-development/adr/0051-full-bitcoin-wallet-v1.md), [ADR-0059](../05-development/adr/0059-self-custody-workstation-product-identity.md)) adds standalone single-signature signing to that list; it does not make signing available now. For generating keys that will hold meaningful value, a dedicated device or an amnesic OS remains an excellent answer — Coldbox is standalone by design and hardware-enhanced by choice, so it is built to make those devices easier to trust without requiring one.
