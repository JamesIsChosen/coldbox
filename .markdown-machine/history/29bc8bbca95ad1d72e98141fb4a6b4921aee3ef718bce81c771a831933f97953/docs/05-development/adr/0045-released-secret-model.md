...
- **The invariant is a declared registry, not a count.** An earlier draft said "exactly two secret-entry points exist in `src/`". That is simply false and would have failed on the day it was written: the sealed realm legitimately holds around two dozen inputs that accept secret material — vault passphrase and confirmation, keyfile, recovery-share re-authentication, recovery-share entry, concealment re-authentication, secret notes, the separate BIP-39 passphrase fields, and every share-combine field that recovery requires. None of those are seed-loading, and none of them should disappear.
  ...
  The registry test also audits the complete form-control surface rather than inferring secret fields from a short list of HTML types. Every `<input>`, `<textarea>`, and `<select>` in `src/**/*.html` declares either `data-input-surface="public"` or `data-input-surface="secret"`; a secret surface is in the sealed document and carries a category that matches this registry. Dynamic cold secret inputs can only be created through the registry-checked factory, while the one detached warm export textarea is explicitly marked public. The test walks every source HTML and JavaScript file and includes negative mutations for an unmarked static control and a new dynamic input, so a future form surface cannot silently escape classification.
