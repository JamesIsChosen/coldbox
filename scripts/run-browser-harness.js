...
async function getColdFrame(page, engine) {
  ...
  // UI.10b: the sealed-realm iframe host collapses behind the status strip's
  // toggle by default now - it is by far the largest of the three boot/
  // security panels' collapsible detail, since it renders the entire cold
  // realm inline. Every one of this function's ~29 call sites needs the
  // frame's own contents to be visible, so expand it once here rather than
  // at each site. This bypasses the toggle's own alarm-refusal gate
  // deliberately: tests need the frame reachable regardless of the strip's
  // UI state, including in the alarm fixtures where it is already forced
  // open anyway.
  await page.evaluate(() => {
    const host = document.getElementById('cold-realm-host');
    if (host) {
      host.hidden = false;
    }
  });
...
async function verifyBuiltFile(browser, engine) {
  ...
  try {
    ...
    // UI.10b: the capability rows collapse behind the status strip's toggle by
    // default now (the panel section itself stays visible, so every check
    // above this one is unaffected) - expand it before asserting a row.
    await page.locator('#status-strip-toggle').click();
...
async function verifyBackupRecordVerification(browser, engine) {
  ...
  try {
    ...
    // UI.10b: Backup Health moved from Home to the Backup page it summarizes,
    // so this check now stays on #backup rather than round-tripping to
    // #dashboard - Home's home-backup-count card is a live-data assertion of
    // its own, covered separately.
    await page.locator('#page-backup:not([hidden])').waitFor({ state: 'visible' });
